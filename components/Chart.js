...
const Chart = ({
  ...
  // to fix bug 1 where if click on one coin and hover and click another coin and unhover, it'll show previous coins price
  const latestCurrentPrice = useSharedValue(currentPrice);
  // useState to wait for charts to load then render
  ...
  useEffect(() => {
    ...
    // takes in a function and takes in miliseconds to wait
    setTimeout(() => {
      setChartReady(true);
    }, 0);
  }, [currentPrice]);
  ...
  const formatUSD = (value) => {
    ...
    const formattedValue = `$${parseFloat(value)
      .toFixed(2)
      .replace(/\d(?=(\d{3})+\.)/g, "$&,")}`;
    // .replace(/\d(?=(\d{3})+\.)/g, '$&,') helps add the "," for example BTC: 33456 -> 33,456
    return formattedValue;
  };
